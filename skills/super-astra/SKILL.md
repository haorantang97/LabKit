...
For an existing installation, check the [upgrade guide](references/rename.md) before installing a second copy; preserve config and review changed hook trust.
...
For ordinary tasks containing `LABKIT_SUPER_ASTRA_ROUTER_V1` or the installed `LABKIT_SUPER_ASTRA_RULE_V1` block (including recognized entries from earlier releases), follow that entry and continue the task without onboarding. On first explicit use without an entry, use [onboarding.md](references/onboarding.md) while completing the task; persistent setup still needs task authorization. Do not just say “available next turn.”
