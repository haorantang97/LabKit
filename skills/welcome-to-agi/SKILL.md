...
If the current turn contains `LABKIT_WELCOME_TO_AGI_ROUTER_V2` or the installed `LABKIT_WELCOME_TO_AGI_RULE_V1` block, follow that entry and continue the task; onboarding is not needed in the task path. Otherwise, on first explicit use or an installation request, read [setup.md](references/setup.md) and [hosts.md](references/hosts.md). Identify the actual client, local/cloud runtime and user/project scope; run `scripts/initialize.py` from the installed folder with those selections to inspect status. Do not just say “available next turn.”

Complete already-authorized installation and initialization using the host adapter. Auto mode prefers persistent rules for known local hosts; hooks are an explicit Codex option. Do not infer the user's interface from having a shell tool or finding an installed executable. If the host is unknown, ask for the client only when needed to configure it; unknown/cloud hosts otherwise get clearly labeled manual mode. Run setup commands for desktop users when tools permit; do not require them to open a terminal for rules mode. Explain the enabled modules, exact target and remaining client steps. An install request authorizes adding our scoped block, not cleaning unrelated rules. If a normal task did not authorize persistent setup, offer setup once while completing that task manually. Honor declined or skill-only setup. Never claim native activation from file registration.
